...
const formattedTime = computed(() => {
  if (!props.event) return ''
  const startTime = new Date(props.event.start_date).toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit'
  })
  const endTime = new Date(props.event.end_date).toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit'
  })
...
const confirmEvent = async () => {
  await updateBookingApprovalState(
    { id: props?.event?.booking_id, ...props.event } as Booking,
    true
  )
    .then(() => {
...
const revokeConfirmation = async () => {
  ...
  dialogStore.show({
    title: t(
      'bookingComponents.editEvent.confirmRevoke',
      'Revoke confirmation'
    ),
    message: t(
      'bookingComponents.editEvent.confirmRevokeMessage',
      'Are you sure you want to revoke the previously approved event?'
    ),
    type: 'warning',
      ...
      <div>
        <div class="flex gap-3 mb-2">
          <h3 class="text-md font-semibold">
            {{ t('bookingComponents.editEvent.status') }}:
          </h3>
          <div class="flex gap-1">
            <p
              :class="{
                'text-green-600': isConfirmed,
                'text-red-600': !isConfirmed
              }"
            >
              {{
                isConfirmed
                  ? t('bookingComponents.editEvent.confirmed')
                  : t('bookingComponents.editEvent.notConfirmed')
              }}
            </p>
            <template v-if="isConfirmed">
              <span class="text-muted-foreground">
                {{ t('bookingComponents.editEvent.by') }}
              </span>
              <span class="text-md font-semibold">{{
                event.confirmed_by_name ||
                event.confirmed_by?.display_name ||
                t('bookingComponents.editEvent.default')
              }}</span>
            </template>
          </div>
        </div>
        ...
        <div v-else class="flex gap-2 mt-2">
          ...
          <Button v-if="canDelete" variant="outline" @click="deleteEvent">
            ...
            {{
              isConfirmed || userCreatedBooking
                ? t('bookingComponents.editEvent.deleteButton')
                : t('bookingComponents.editEvent.rejectButton')
            }}
          </Button>
        </div>
      </div>
      <div class="flex flex-col gap-2">
          ...
          <div class="ps-3 flex flex-col gap-2 border-l-2 -mt-2.5 pt-2.5">
            ...
            >
              <Label class="text-muted-foreground"
                >{{ t('bookingComponents.editEvent.title') }}:</Label
              >
              <Input
                :model-value="event.display_name"
                v-if="event.display_name"
                readonly
              />
              <Input
                :model-value="event.booking_display_name"
                v-else
                readonly
              />
            </div>
            ...
            >
              <Label class="text-muted-foreground"
                >{{ t('bookingComponents.editEvent.email') }}:</Label
              >
            ...
            >
              <Label class="text-muted-foreground"
                >{{ t('bookingComponents.editEvent.phone') }}:</Label
              >
        ...
        >
          <Label class="text-md font-semibold">{{
            t('bookingComponents.editEvent.description')
          }}</Label>
    ...
    <DialogContent class="sm:max-w-[450px]">
      <DialogHeader>
        <DialogTitle>
          ...
          <span v-if="!event?.is_full_day">
            {{ t('bookingComponents.editEvent.from') }}
            {{ formattedTime }}</span
          >
      ...
      <DrawerFooter class="pt-2">
        <DrawerClose as-child>
          <Button variant="outline">
            {{ t('bookingComponents.editEvent.cancelButton') }}
          </Button>
        </DrawerClose>
      </DrawerFooter>
    </DrawerContent>
